...
//---------------------------------------------------------
//---------------------------------------------------------
void CAbhDemon::OnScheduleChange(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CAbhDemon::SelectSchedule(void)
{
	...
	if (HasCondition(COND_ABHDEMON_CLIMB_TOUCH))
	{
		return SCHED_ABHDEMON_UNSTICK_JUMP;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAbhDemon::PrescheduleThink(void)
{
	...
	if (m_pMoanSound && gpGlobals->curtime > m_flTimeUpdateSound)
	{
		...
		{
			// Calm down!
			flDistNoBBox = ABHDEMON_EXCITE_DIST;
			m_flTimeUpdateSound += 1.0;
		}

		if (flDistNoBBox >= ABHDEMON_EXCITE_DIST && m_flDistFactor != 1.0)
		{
			// Go back to normal pitch.
			m_flDistFactor = 1.0;

			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, ABHDEMON_IDLE_PITCH, ABHDEMON_SOUND_UPDATE_FREQ);
		}
		else if (flDistNoBBox < ABHDEMON_EXCITE_DIST)
		{
			// Zombie is close! Recalculate pitch.
			int iPitch;

			m_flDistFactor = MIN(1.0, 1 - flDistNoBBox / ABHDEMON_EXCITE_DIST);
			iPitch = ABHDEMON_MIN_PITCH + ((ABHDEMON_MAX_PITCH - ABHDEMON_MIN_PITCH) * m_flDistFactor);
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, iPitch, ABHDEMON_SOUND_UPDATE_FREQ);
		}

		m_flTimeUpdateSound = gpGlobals->curtime + ABHDEMON_SOUND_UPDATE_FREQ;
	}
	...
	if (IsCurSchedule(SCHED_ABHDEMON_RANGE_ATTACK1, false))
	{
...
//
//-----------------------------------------------------------------------------
void CAbhDemon::SoundInit(void)
{
	if (!m_pMoanSound)
	{
		// !!!HACKHACK - kickstart the moan sound. (sjb)
		MoanSound(envAbhDemonMoanVolume, ARRAYSIZE(envAbhDemonMoanVolume));
...
//-----------------------------------------------------------------------------
void CAbhDemon::SetIdleSoundState(void)
{
	// Main looping sound
	if (m_pMoanSound)
	{
		ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, ABHDEMON_IDLE_PITCH, 1.0);
...
//-----------------------------------------------------------------------------
void CAbhDemon::SetAngrySoundState(void)
{
	...
	// Main looping sound
	ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, ABHDEMON_MIN_PITCH, 0.5);
...
//-----------------------------------------------------------------------------
const char *CAbhDemon::GetHeadcrabClassname(void)
{
	return "npc_headcrab_fast";
}

const char *CAbhDemon::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CAbhDemon::MaxYawSpeed(void)
{
	switch (GetActivity())
	{
	case ACT_TURN_LEFT:
...
//-----------------------------------------------------------------------------
const char *CAbhDemon::GetLegsModel(void)
{
	return s_pLegsModel;
}

const char *CAbhDemon::GetTorsoModel(void)
{
...
//
//-----------------------------------------------------------------------------
int CAbhDemon::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (!GetEnemy())
	{
		return COND_NONE;
...
//-----------------------------------------------------------------------------
const char *CAbhDemon::GetMoanSound(int nSound)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CAbhDemon::AttackHitSound(void)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::AttackMissSound(void)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::LeapAttackSound(void)
{
	EmitSound("NPC_FastZombie.LeapAttack");
}
...
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CAbhDemon::AttackSound(void)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::PainSound(const CTakeDamageInfo &info)
{
	if (m_pLayer2)
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envAbhDemonVolumePain, ARRAYSIZE(envAbhDemonVolumePain));
	if (m_pMoanSound)
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, envAbhDemonInverseVolumePain, ARRAYSIZE(envAbhDemonInverseVolumePain));
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAbhDemon::DeathSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::AlertSound(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define ABHDEMON_MINLEAP			200
#define ABHDEMON_MAXLEAP			300
float CAbhDemon::InnateRange1MaxRange(void)
{
	return ABHDEMON_MAXLEAP;
}
...
//
//-----------------------------------------------------------------------------
int CAbhDemon::RangeAttack1Conditions(float flDot, float flDist)
{
...
void CAbhDemon::HandleAnimEvent(animevent_t *pEvent)
{
	if (pEvent->event == AE_ABHDEMON_CLIMB_LEFT || pEvent->event == AE_ABHDEMON_CLIMB_RIGHT)
	{
		if (++m_iClimbCount % 3 == 0)
		{
			...
			ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envAbhDemonVolumeClimb, ARRAYSIZE(envAbhDemonVolumeClimb));
		}
...
//
//-----------------------------------------------------------------------------
void CAbhDemon::LeapAttack(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAbhDemon::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ABHDEMON_VERIFY_ATTACK:
	...
	case TASK_ABHDEMON_UNSTICK_JUMP:
	{
		SetGroundEntity(NULL);
	...
	case TASK_ABHDEMON_LAND_RECOVER:
	{
		// Set the ideal yaw
		Vector flEnemyLKP = GetEnemyLKP();
		GetMotor()->SetIdealYawToTarget(flEnemyLKP);
	...
	case TASK_RANGE_ATTACK1:
		...
		SetTouch(&CAbhDemon::LeapAttackTouch);
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAbhDemon::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ABHDEMON_JUMP_BACK:
...
int CAbhDemon::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_RANGE_ATTACK1:
	{
		...
		return SCHED_ABHDEMON_RANGE_ATTACK1;
	}
	break;
	...
	case SCHED_MELEE_ATTACK1:
		if (m_fIsTorso == true)
		{
			return SCHED_ABHDEMON_TORSO_MELEE_ATTACK1;
		}
		else
		{
			return SCHED_ABHDEMON_MELEE_ATTACK1;
		}
		break;

	case SCHED_ABHDEMON_UNSTICK_JUMP:
		if (GetActivity() == ACT_CLIMB_UP || GetActivity() == ACT_CLIMB_DOWN || GetActivity() == ACT_CLIMB_DISMOUNT)
		{
			return SCHED_ABHDEMON_CLIMBING_UNSTICK_JUMP;
		}
		else
		{
			return SCHED_ABHDEMON_UNSTICK_JUMP;
...
//---------------------------------------------------------
//---------------------------------------------------------
Activity CAbhDemon::NPC_TranslateActivity(Activity baseAct)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAbhDemon::LeapAttackTouch(CBaseEntity *pOther)
{
	if (!pOther->IsSolid())
	{
...
//-----------------------------------------------------------------------------
void CAbhDemon::ClimbTouch(CBaseEntity *pOther)
{
	if (pOther->IsPlayer())
	{
		...
		{
			SetCondition(COND_ABHDEMON_CLIMB_TOUCH);
		}
...
//-----------------------------------------------------------------------------
void CAbhDemon::StopLoopingSounds(void)
{
	if (m_pMoanSound)
	{
...
//-----------------------------------------------------------------------------
void CAbhDemon::BecomeTorso(const Vector &vecTorsoForce, const Vector &vecLegsForce)
{
...
// Output :
//-----------------------------------------------------------------------------
bool CAbhDemon::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
bool CAbhDemon::MovementCost(int moveType, const Vector &vecStart, const Vector &vecEnd, float *pCost)
{
...
bool CAbhDemon::ShouldFailNav(bool bMovementFailed)
{
...
//---------------------------------------------------------
void CAbhDemon::OnNavJumpHitApex(void)
{
...
void CAbhDemon::OnChangeActivity(Activity NewActivity)
{
	...
	else if (GetActivity() == ACT_GLIDE)
	{
		...
		if (m_pMoanSound)
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, ABHDEMON_MIN_PITCH, 0.3);
	}
...
// 
//=========================================================
int CAbhDemon::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	if (m_fJustJumped)
	{
...
//=========================================================
void CAbhDemon::BeginNavJump(void)
{
	m_fIsNavJumping = true;
	m_fHitApex = false;

	ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envAbhDemonVolumeJump, ARRAYSIZE(envAbhDemonVolumeJump));
}

//=========================================================
// 
//=========================================================
void CAbhDemon::EndNavJump(void)
{
...
// 
//=========================================================
void CAbhDemon::BeginAttackJump(void)
{
...
// 
//=========================================================
void CAbhDemon::EndAttackJump(void)
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAbhDemon::BuildScheduleTestBits(void)
{
	...
	if (GetActivity() == ACT_CLIMB_UP || GetActivity() == ACT_CLIMB_DOWN || GetActivity() == ACT_CLIMB_DISMOUNT)
	{
		SetCustomInterruptCondition(COND_ABHDEMON_CLIMB_TOUCH);
	}
	else
	{
		ClearCustomInterruptCondition(COND_ABHDEMON_CLIMB_TOUCH);
	}
}
...
// 
//=========================================================
void CAbhDemon::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAbhDemon::Event_Killed(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAbhDemon::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
	if (m_fIsTorso)
	{
		// Already split.
...
//-----------------------------------------------------------------------------
bool CAbhDemon::CreateBehaviors(void)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::InputAttachToVehicle(inputdata_t &inputdata)
{
	// Interrupt us
...
//-----------------------------------------------------------------------------
void CAbhDemon::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::VehicleLeapAttack(void)
{
	CBaseEntity *pEnemy = GetEnemy();
	if (pEnemy == NULL)
		return;
	...
	SetTouch(&CAbhDemon::VehicleLeapAttackTouch);
}
...
//-----------------------------------------------------------------------------
bool CAbhDemon::CanEnterVehicle(CPropJeepEpisodic *pVehicle)
{
	if (pVehicle == NULL)
		return false;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CAbhDemon::VehicleLeapAttackTouch(CBaseEntity *pOther)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAbhDemon::IsInAVehicle(void)
{
...
//-----------------------------------------------------------------------------
void CAbhDemon::UpdateEfficiency(bool bInPVS)
{
...
DECLARE_TASK(TASK_ABHDEMON_UNSTICK_JUMP)
DECLARE_TASK(TASK_ABHDEMON_JUMP_BACK)
DECLARE_TASK(TASK_ABHDEMON_VERIFY_ATTACK)
...
DEFINE_SCHEDULE
(
SCHED_ABHDEMON_RANGE_ATTACK1,

"	Tasks"
"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_RANGE_ATTACK1"
"		TASK_SET_ACTIVITY				ACTIVITY:ACT_ABHDEMON_LEAP_STRIKE"
...
"		TASK_MELEE_ATTACK1				0"
"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_ABHDEMON_FRENZY"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CHASE_ENEMY"
"		TASK_ABHDEMON_VERIFY_ATTACK	0"
"		TASK_PLAY_SEQUENCE_FACE_ENEMY	ACTIVITY:ACT_ABHDEMON_BIG_SLASH"
