										...
										IShaderShadow* pShaderShadow, DrawFlashlight_dx90_Vars_t &vars )
{
	...
	if( pShaderShadow )
	{
		...
		if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			...
			pshIndex.SetPHONG(vars.m_nPhongVar != 0);
	...
	else
	{
		...
		if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			...
			if (params[vars.m_nPhongVar]->GetIntValue() != 0)
			{
				float phong[4];
				phong[0] = params[vars.m_nPhongExponent]->GetFloatValue();
				phong[1] = params[vars.m_nPhongBoost]->GetFloatValue();
				phong[2] = 0.0f;
				phong[3] = 0.0f;
				s_pShaderAPI->SetPixelShaderConstant(PSREG_SPEC_RIM_PARAMS, phong, 1);
			}
		}
		else
		{
		...
		float pos[4];										// Set the flashlight position
		pos[0] = flashlightState.m_vecLightOrigin.x;
		pos[1] = flashlightState.m_vecLightOrigin.y;
		pos[2] = flashlightState.m_vecLightOrigin.z;
		pos[3] = 0.0f;
		s_pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_POSITION, pos, 1);
